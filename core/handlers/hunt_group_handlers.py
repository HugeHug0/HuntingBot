...
@check_user_registration(only_registered=True)
async def hunt_group_handlers(callback: CallbackQuery):
    tg_id = callback.from_user.id
    forum_link = await get_hunt_group_link_or_none(tg_id)

    if forum_link:
        await callback.message.edit_text(message_texts.hunt_group_join, reply_markup=hunt_group_link_inline_keyboard(forum_link))
    else:
        await callback.message.edit_text(message_texts.hunt_group_request, reply_markup=hunt_group_request_inline_keyboard())
...
async def hunt_group_confirm_callback_query_handler(callback: CallbackQuery):
    ...
    new_hunt_group_link = await callback.bot.create_chat_invite_link(
        chat_id=settings.bots.hunt_group_id,
        expire_date=None,  # бессрочная
        member_limit=1  # одноразовая
    )

    tg_id = await hunt_group_update_link_and_get_hunter_tg_id(msg_id, new_hunt_group_link.invite_link)
